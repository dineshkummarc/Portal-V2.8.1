...
namespace Portal.Modules.FileBrowser
{
  ...
  public class ConfigAgentz
  {
    ...
    public DirectoryWrapper EditDirectory
    {
      get
      {
        if (null == editDirectory)
        {
          // Es ist noch kein aktuelles Verzeichnis vorgewählt. Darum wird das Aktuelle Verzeichnis verwendet.
          editDirectory = CurrentDirectory;
        }
    ...
    protected string PhysicalRoot
    {
      get
      {
        return HttpContext.Current.Server.MapPath(config.VirtualRoot);
      }
    }  
