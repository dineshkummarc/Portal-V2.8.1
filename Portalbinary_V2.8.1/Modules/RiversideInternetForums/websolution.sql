IF EXISTS (SELECT name FROM master.dbo.sysdatabases WHERE name = N'RiversideInternetForums')
DROP DATABASE [RiversideInternetForums]
GO

CREATE DATABASE [RiversideInternetForums]
...
if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[WS_Posts]') and OBJECTPROPERTY(id, N'IsUserTable') = 1)
drop table [dbo].[WS_Posts]
GO

if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[WS_Threads]') and OBJECTPROPERTY(id, N'IsUserTable') = 1)
drop table [dbo].[WS_Threads]
GO

if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[WS_Users]') and OBJECTPROPERTY(id, N'IsUserTable') = 1)
drop table [dbo].[WS_Users]
GO

if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[WS_Webs]') and OBJECTPROPERTY(id, N'IsUserTable') = 1)
drop table [dbo].[WS_Webs]
GO

CREATE TABLE [dbo].[WS_Posts] (
	[PostID] [int] IDENTITY (1, 1) NOT NULL ,
	[ParentPostID] [int] NOT NULL ,
	[UserID] [int] NOT NULL ,
	[RemoteAddr] [nvarchar] (100) COLLATE SQL_Latin1_General_CP1_CI_AS NOT NULL ,
	[Notify] [bit] NOT NULL ,
	...
	[TreeSortOrder] [int] NOT NULL ,
	[FlatSortOrder] [int] NOT NULL 
) ON [PRIMARY] TEXTIMAGE_ON [PRIMARY]
GO

CREATE TABLE [dbo].[WS_Threads] (
	[ThreadID] [int] NOT NULL ,
	[Replies] [int] NOT NULL ,
	[Views] [int] NOT NULL ,
	[LastPostedPostID] [int] NOT NULL ,
	[ForumID] [int] NOT NULL ,
	[PinnedDate] [datetime] NOT NULL 
) ON [PRIMARY]
GO

CREATE TABLE [dbo].[WS_Users] (
...
ALTER TABLE [dbo].[WS_Threads] WITH NOCHECK ADD 
...
ALTER TABLE [dbo].[WS_Users] WITH NOCHECK ADD 
...
ALTER TABLE [dbo].[WS_Webs] WITH NOCHECK ADD 
	CONSTRAINT [PK_Webs] PRIMARY KEY  CLUSTERED 
	(
		[WebDomain]
	)  ON [PRIMARY] 
GO

 CREATE  UNIQUE  CLUSTERED  INDEX [IX_Posts_PostID] ON [dbo].[WS_Posts]([PostID]) ON [PRIMARY]
GO

ALTER TABLE [dbo].[WS_Posts] WITH NOCHECK ADD 
	CONSTRAINT [DF_Posts_ThreadID] DEFAULT (0) FOR [ThreadID],
	CONSTRAINT [DF_Posts_PostLevel] DEFAULT (0) FOR [PostLevel],
	CONSTRAINT [DF_Posts_TreeSortOrder] DEFAULT (0) FOR [TreeSortOrder],
	CONSTRAINT [DF_Posts_FlatSortOrder] DEFAULT (0) FOR [FlatSortOrder],
...
ALTER TABLE [dbo].[WS_Threads] WITH NOCHECK ADD 
	...
	CONSTRAINT [DF_Threads_ForumID] DEFAULT (0) FOR [ForumID]
GO

ALTER TABLE [dbo].[WS_Users] WITH NOCHECK ADD 
	CONSTRAINT [DF_Users_PostCount] DEFAULT (0) FOR [PostCount]
GO

 CREATE  INDEX [IX_Posts_ParentPostID] ON [dbo].[WS_Posts]([ParentPostID]) ON [PRIMARY]
GO

 CREATE  INDEX [IX_Posts_ThreadID] ON [dbo].[WS_Posts]([ThreadID]) ON [PRIMARY]
GO

 CREATE  INDEX [IX_Posts_TreeSortOrder] ON [dbo].[WS_Posts]([TreeSortOrder]) ON [PRIMARY]
GO

 CREATE  INDEX [IX_Posts_FlatSortOrder] ON [dbo].[WS_Posts]([FlatSortOrder]) ON [PRIMARY]
GO

 CREATE  INDEX [IX_Threads_ForumID] ON [dbo].[WS_Threads]([ForumID]) ON [PRIMARY]
GO

 CREATE  INDEX [IX_Threads_LastPostedPostID] ON [dbo].[WS_Threads]([LastPostedPostID]) ON [PRIMARY]
GO

 CREATE  INDEX [IX_Threads_ThreadID] ON [dbo].[WS_Threads]([ThreadID]) ON [PRIMARY]
GO

 CREATE  INDEX [IX_Users_WebID] ON [dbo].[WS_Users]([WebID]) ON [PRIMARY]
GO
...
SELECT @UserID = (
	...
	WHERE
		...
		WebID = @WebID)
GO
CREATE PROCEDURE WS_GetUserIDFromAlias
(
	@Alias	nvarchar(100),
	@WebID	int,
	@UserID	int OUTPUT
...
INSERT INTO WS_Webs VALUES('localhost', 1, NULL)
GO
